...
with tf.Session() as sess:
    ...
    for i in range(2000):
        ...
        step, logits, loss, acc , summaries= sess.run([train_step, train_logits, train_loss, train_acc, summary_op],
        ...
        if i % 1000 == 0:
            ...
            res_dict,res_list = fetch_error_distribution(acc[1])
            print('{}'.format(res_dict))
            print('\n'+'---------------------------------')
            print('{}'.format(res_list))

            saver.save(sess,save_path = './model/des', global_step=i)
    coord.request_stop()
    # coord.join(threads)
    sess.close()
